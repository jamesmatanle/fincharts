...
sync_conf() {
    ...
    ssh -t $SERVER 'sudo mv /tmp/conf /etc/nginx/sites-available/fincharts && sudo ln -sf /etc/nginx/sites-available/fincharts /etc/nginx/sites-enabled/ && sudo rm -f /etc/nginx/sites-enabled/jamesmatanle.com && sudo nginx -t && sudo systemctl restart nginx'
}
